class Input{
    ...
    setRange(minNum,maxNum){
        this.range.min = minNum;
        this.range.max = maxNum;
    }
    ...
    //returns: nothing
    //saves the given pattern into the object  /^[0-9]*$/
    setPattern( regex ){
        this.pattern = regex;
    }
    ...
    /*notes:
        let's say your property for your regex pattern was this.regex,
        and your property for your input was this.targetInput
        get the value from the targetInput (for example, this.targetInput.val()) and it put in a variable,
    ...
    test(){
        ...
        if (this.range.min || this.range.max) {
            if(this.targetInput.val() > this.range.min && this.targetInput.val() < this.range.max){
                this.resultObj.result = true;
            }else{
                this.resultObj.result = false;
                this.resultObj.error = 'range';
            }
        } else {
            if (!this.pattern || this.pattern.test(this.targetInput.val())) {
                this.resultObj.result = true;
            } else {
                this.resultObj.error = 'pattern';
                this.resultObj.result = false;
            }
        }
    ...
    showError( messageStr ){
        this.errorElement = $('<div>')
            .css({
                'left': this.targetInput.position().left,
                'top': this.targetInput.height() + this.targetInput.position().top
            })
            .addClass('inputError')
            .text(messageStr)

        $(this.targetInput).parent().append(this.errorElement);
    }
    // /*
